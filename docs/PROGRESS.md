...
- [ ] **T1. Ingestion parser decision** — test whether **Docling** installs & runs on this machine (no GPU, 16GB RAM); compare heading extraction + speed vs MarkItDown on `Level 1 Lessons.pdf`. Pick the default (user prefers Docling if feasible).
